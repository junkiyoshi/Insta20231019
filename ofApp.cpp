...
void ofApp::update() {
	...
	this->setRingToMesh(this->face, this->frame, glm::vec3(), 0, 140, 30, 0, 360, noise_seed);
	for (auto radius = 120; radius < 340; radius += 10) {

		this->setRingToMesh(this->face, this->frame, glm::vec3(), radius, 8, 60, 90, 180, noise_seed);
		this->setRingToMesh(this->face, this->frame, glm::vec3(), radius, 8, 60, 270, 360, noise_seed);
	}
}
...
void ofApp::setRingToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float radius, float width, float height, int deg_start, int deg_end, glm::vec2 noise_seed) {
	...
	auto noise_value = radius != 0 ? glm::vec2(ofNoise(noise_seed.x - radius * 0.0035 + ofGetFrameNum() * 0.005), ofNoise(noise_seed.y - radius * 0.0035 + ofGetFrameNum() * 0.005)) :
		glm::vec2(ofNoise(noise_seed.x - 100 * 0.005 + ofGetFrameNum() * 0.005), ofNoise(noise_seed.y - 100 * 0.005 + ofGetFrameNum() * 0.005));
